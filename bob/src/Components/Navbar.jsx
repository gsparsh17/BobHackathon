...
const Navbar = ({ navigationData, currentRoute, setCurrentRoute }) => {
  ...
  function handleNavigation(item) {
    if (item === "Discover") {
      ...
      window.location.href = "https://stockprediction-nbpbmficpawpv4ybpl5jwe.streamlit.app/"; // Replace with your URL
    } else {
  ...
  return (
    <div className="bg-orange-50 z-40">
      <nav className="hidden md:flex flex-row items-center justify-between px-8 pb-10 h-12 pt-12 rounded-b-2xl">
        ...
        <button
          onClick={up1}
          className="hover:bg-gray-50 border-2 border-red-500 text-sm text-yellow-500 py-3 px-5 rounded-lg font-medium tracking-wide leading-none"
        >
          Register
        </button>
      </nav>
